...
Your STRIP exposure: STRIP held in your wallet, STRIP staked in stSTRIP, and the STRIP inside your staked sWLP positions across every vault's pool. Staking Principal Tokens earns you the emissions that boost multiplies, but the act of staking PT itself does not move the clock, and unclaimed rewards do not count either. Exposure starts when STRIP is claimed and kept. During the [Accrual Period](/launch-phases), before STRIP exists, the multiplier ramps from 1x toward 5x based on time staked, and the level you reach carries forward once claims open.
...
In two layers, and emissions only ever flow to staking venues, never to vaults themselves. First, each vault's performance, **APR × TVL**, measures the combined share of emissions its two venues receive, so the vaults earning the most yield for the system command the largest budgets. Then that budget splits between the vault's PT staking and PT/STRIP liquidity venues following the published policy on [Launch Phases](/launch-phases): all-staking during the Accrual Period, roughly half-and-half while claimed STRIP builds market depth, then a depth-targeted split in steady state. The second layer never moves emissions between one vault's venues and another's. Split changes among registered venues execute immediately and are logged onchain; adding or removing pools requires a 24-hour timelock.
